...
namespace Business.Repositories.UserOperationClaimRepository
{
    public class UserOperationClaimManager : IUserOperationClaimService
    {
        ...
        {
            IResult result = BusinessRules.Run(await IsUserExist(userOperationClaim.UserId), await IsOperationClaimExist(userOperationClaim.OperationClaimId), await IsOperationSetExistForAdd(userOperationClaim));
            if (result != null)
            {
                return result;
            }
        ...
        {
            IResult result = BusinessRules.Run(await IsUserExist(userOperationClaim.UserId), await IsOperationClaimExist(userOperationClaim.OperationClaimId), await IsOperationSetExistForUpdate(userOperationClaim));
            if (result != null)
            {
                return result;
            }
        ...
        private async Task<IResult> IsOperationSetExistForAdd(UserOperationClaim userOperationClaim)
        {
        ...
        private async Task<IResult> IsUserExist(int userId)
        {
            var result = _userService.GetById(userId);

            if (result == null)
            {
                return new ErrorResult(UserMessages.UserNotExist);
            }

            return new SuccessResult();
        }
        private async Task<IResult> IsOperationClaimExist(int operationClaimId)
        {
